...
class cv32e20(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dut_exe = os.path.join(config['PATH'] if 'PATH' in config else "","cv32e20")
    ...
    def initialise(self, suite, work_dir, archtest_env):
       ...
       execute = 'make -C ./plugin-cv32e20/tb verilate'
       logger.debug('DUT executing ' + execute)
       utils.shellCommand(execute).run()
...
#The following template only uses shell commands to compile and run the tests.
    ...
    def runTests(self, testList):
      ...
      for testname in testList:
          ...
          # cv32e20-specific - dirrrty shell stuff! ;)
          execute = 'make -C plugin-cv32e20/tb veri-test TEST_PROGRAM_RELPATH=../../{0} TEST={1}'.format(rel_test_dir, elf)
          logger.info('DUT executing: ' + execute)
          utils.shellCommand(execute).run()

          execute = 'cp ./plugin-cv32e20/tb/main.sig {0}/DUT-cv32e20.signature'.format(rel_test_dir)
          logger.info('DUT executing: ' + execute)
          utils.shellCommand(execute).run()
